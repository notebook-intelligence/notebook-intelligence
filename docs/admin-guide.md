...
**Reading the numbers.** In a turn's summary, compare `total` against `api` (the SDK-reported model/API duration): when they are close, the time is in the gateway or model; when they diverge, look at `connect` (subprocess/CLI cold start, typically node startup plus bundle reads on slow storage), `context_prep` (per-turn rule and skill discovery on the server's home filesystem), and `stalls` (mid-stream gaps, tagged by whether they followed tool use or text). For filesystem suspicion, the probe's fsync latency and sustained-throughput rows for `~/.claude` are the EFS signals; sustained throughput of a few MB/s with high fsync latency on an `nfs4` mount is the burst-credit-exhaustion signature, which is confirmed from CloudWatch (`BurstCreditBalance`) rather than from inside the pod.

**Data governance.** The recorder enforces a fixed attribute allowlist; prompt/response text, file contents, absolute paths, env var values, exception messages, and hostnames are never recorded. Default `attr_detail: redacted` additionally hashes file basenames and model/tool/server names. Probe output replaces home paths with `~` and carries a `contains_internal_hostnames` flag when the network check ran, since the configured gateway host appears in its own rows.
